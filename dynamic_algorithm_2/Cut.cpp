...
ostream& operator<<(ostream& c, Cut& cut)
{
	...
	if (cut.type == SEGMENT_INTERSECTION)
	{
		c << *cut.intersecting_segment;
	}

	return c;
}

bool Cut::compare_cuts(Cut* other_cut)
{
	//cout << "comapre cuts\n";
	if (type == other_cut->type && segment->seg == other_cut->segment->seg)
	{
		...
		{
			if (intersecting_segment->seg == other_cut->intersecting_segment->seg && intersecting_segment->priority == other_cut->intersecting_segment->priority) 
			{
				return true;
			}
			else 
			{
				std::cout << "\n intersection cuts with distinct intersecting segments \n";
				return false;
			}
		}
	}
	
	if (type == other_cut->type) 
	{
		cout << "\n cuts have the same type but different defining segments... \n";
	}

	cout << *this << "\n";
	cout << segment->seg << "\n";
	cout << *other_cut << "\n";
	cout << other_cut->segment->seg << "\n";
	return false;
}
